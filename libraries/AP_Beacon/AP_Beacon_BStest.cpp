...
AP_Beacon_BStest::AP_Beacon_BStest(AP_Beacon &frontend, AP_SerialManager &serial_manager) : 
    ...
    while (nbytes-- > 0)
    {
        ...
        if (b >= 0)
        {
            ...
            if (type == MsgType::AOA_Node_Frame0)
            {
                ...
                // gcs().send_text(MAV_SEVERITY_DEBUG, "test from AP_Beacon_BStest");
            }
...
AP_Beacon_BStest::MsgType AP_Beacon_BStest::parse_byte(uint8_t b)
{
...
// 拿到dis和angel
void AP_Beacon_BStest::parse_aoa_node_frame0()
{
    // 有接收到block才解析
    if (_frame_len > 4+17)
    {
